# SwitchArray生產
+ 更新日期: 2025-06-06

### 燒錄方式
+ 網址: https://cgrg.synology.me/ControlBoard_Firmare


### CP210x修改序號工具
+ 檔案名稱: CP210x_LegacyUtilities.zip
+ 下載頁面: https://community.silabs.com/s/article/cp210x-legacy-programming-utilities?language=en_US

### SwitchArray 2025-S003
+ SwitchArray完整序號為: 2025-S003
+ 使用「MO系列金屬系統機殼 MO99-16-23B」，並新增支架與面板開孔。
+ 貼上標籤貼紙「2025-S003」。
+ 準備外箱，並貼上貼紙。
+ 準備配件包:
  + USB2.0數據線(透明藍)_1.5m__A公對A公 x1
  + PSU連接線(橘、灰)_槍型接頭轉防水接頭 x1
  + DMM連接線(紅、黑)_槍型接頭公對公 x1
+ SwitchArray內部使用的「ESP32」一律改用「CP2102」版本
  + 使用CP210x修改序號工具將序號修改為: 「2025-S003」
    + 插上裝置後(避免弄錯一次只插上一個CP210x裝置)，運行「CP210xSetIDs.exe」，找到中間有一個格子有標註「Serial Number」，填入序號「2025-S003」，在把前面的勾選框勾起來，最後按下最下方的「Program Device」，完成後關閉修改工具，並反覆插拔裝置與檢查序號是否正確。
  + 修改成功後，可在裝置管理員>連接埠(COM和LPT)>Silicon Labs CP210x USB to UART Bridge (COM3)>詳細資料> 裝置例項路徑，會看到「USB\VID_10C4&PID_EA60\2025-S003」。其中「2025-S003」就是SwitchArray完整序號。
+ SwitchArray內部裝有「USB3.0 HUB」與「USB2.0 32GB USBDISK」
  + USBDISK:
    + 容量: 32[GB]
    + 格式: exFAT
    + 磁碟機名稱: 與序號相同，例如2025-S003
    + 放置相關軟體至資料夾中
      + R2MS_Lite_安裝軟體:
        + NCUF
        + R2MS_Lite_Driver
      + 其他安裝軟體
        + RustDesk
+ 空接放電測試：
  + 主要關心電壓為相似整齊波型，電流為0[A]。
  + 2025-06-06通過測試
+ 10歐姆模擬地層放電測試:
  + 主要關心電壓為相似整齊波型，電流為0.1[A]。
  + 2025-06-06通過測試

### SwitchArray 2025-S004
+ SwitchArray完整序號為: 2025-S004
+ 使用「MO系列金屬系統機殼 MO99-16-23B」，並新增支架與面板開孔。
+ 貼上標籤貼紙「2025-S004」。
+ 準備外箱，並貼上貼紙。
+ 準備配件包:
  + USB2.0數據線(透明藍)_1.5m__A公對A公 x1
  + PSU連接線(橘、灰)_槍型接頭轉防水接頭 x1
  + DMM連接線(紅、黑)_槍型接頭公對公 x1
+ SwitchArray內部使用的「ESP32」一律改用「CP2102」版本
  + 使用CP210x修改序號工具將序號修改為: 「2025-S004」
    + 插上裝置後(避免弄錯一次只插上一個CP210x裝置)，運行「CP210xSetIDs.exe」，找到中間有一個格子有標註「Serial Number」，填入序號「2025-S004」，在把前面的勾選框勾起來，最後按下最下方的「Program Device」，完成後關閉修改工具，並反覆插拔裝置與檢查序號是否正確。
  + 修改成功後，可在裝置管理員>連接埠(COM和LPT)>Silicon Labs CP210x USB to UART Bridge (COM3)>詳細資料> 裝置例項路徑，會看到「USB\VID_10C4&PID_EA60\2025-S004」。其中「2025-S004」就是SwitchArray完整序號。
+ SwitchArray內部裝有「USB3.0 HUB」與「USB2.0 32GB USBDISK」
  + USBDISK:
    + 容量: 32[GB]
    + 格式: exFAT
    + 磁碟機名稱: 與序號相同，例如2025-S004
    + 放置相關軟體至資料夾中
      + R2MS_Lite_安裝軟體:
        + NCUF
        + R2MS_Lite_Driver
      + 其他安裝軟體
        + RustDesk
+ 空接放電測試：
  + 主要關心電壓為相似整齊波型，電流為0[A]。
  + 2025-06-06測試失敗
+ 10歐姆模擬地層放電測試:
  + 主要關心電壓為相似整齊波型，電流為0.1[A]。
  + 2025-06-06測試失敗

### SwitchArray 2025-S005
+ SwitchArray完整序號為: 2025-S005
+ 使用「MO系列金屬系統機殼 MO99-16-23B」，並新增支架與面板開孔。
+ 貼上標籤貼紙「2025-S005」。
+ 準備外箱，並貼上貼紙。
+ 準備配件包:
  + USB2.0數據線(透明藍)_1.5m__A公對A公 x1
  + PSU連接線(橘、灰)_槍型接頭轉防水接頭 x1
  + DMM連接線(紅、黑)_槍型接頭公對公 x1
+ SwitchArray內部使用的「ESP32」一律改用「CP2102」版本
  + 使用CP210x修改序號工具將序號修改為: 「2025-S005」
    + 插上裝置後(避免弄錯一次只插上一個CP210x裝置)，運行「CP210xSetIDs.exe」，找到中間有一個格子有標註「Serial Number」，填入序號「2025-S005」，在把前面的勾選框勾起來，最後按下最下方的「Program Device」，完成後關閉修改工具，並反覆插拔裝置與檢查序號是否正確。
  + 修改成功後，可在裝置管理員>連接埠(COM和LPT)>Silicon Labs CP210x USB to UART Bridge (COM3)>詳細資料> 裝置例項路徑，會看到「USB\VID_10C4&PID_EA60\2025-S005」。其中「2025-S005」就是SwitchArray完整序號。
+ SwitchArray內部裝有「USB3.0 HUB」與「USB2.0 32GB USBDISK」
  + USBDISK:
    + 容量: 32[GB]
    + 格式: exFAT
    + 磁碟機名稱: 與序號相同，例如2025-S005
    + 放置相關軟體至資料夾中
      + R2MS_Lite_安裝軟體:
        + NCUF
        + R2MS_Lite_Driver
      + 其他安裝軟體
        + RustDesk
+ 空接放電測試：
  + 主要關心電壓為相似整齊波型，電流為0[A]。
  + 2025-06-06通過測試
+ 10歐姆模擬地層放電測試:
  + 主要關心電壓為相似整齊波型，電流為0.1[A]。
  + 2025-06-06通過測試

### SwitchArray 2025-S006
+ SwitchArray完整序號為: 2025-S006
+ 使用「MO系列金屬系統機殼 MO99-16-23B」，並新增支架與面板開孔。
+ 貼上標籤貼紙「2025-S006」。
+ 準備外箱，並貼上貼紙。
+ 準備配件包:
  + USB2.0數據線(透明藍)_1.5m__A公對A公 x1
  + PSU連接線(橘、灰)_槍型接頭轉防水接頭 x1
  + DMM連接線(紅、黑)_槍型接頭公對公 x1
+ SwitchArray內部使用的「ESP32」一律改用「CP2102」版本
  + 使用CP210x修改序號工具將序號修改為: 「2025-S006」
    + 插上裝置後(避免弄錯一次只插上一個CP210x裝置)，運行「CP210xSetIDs.exe」，找到中間有一個格子有標註「Serial Number」，填入序號「2025-S006」，在把前面的勾選框勾起來，最後按下最下方的「Program Device」，完成後關閉修改工具，並反覆插拔裝置與檢查序號是否正確。
  + 修改成功後，可在裝置管理員>連接埠(COM和LPT)>Silicon Labs CP210x USB to UART Bridge (COM3)>詳細資料> 裝置例項路徑，會看到「USB\VID_10C4&PID_EA60\2025-S006」。其中「2025-S006」就是SwitchArray完整序號。
+ SwitchArray內部裝有「USB3.0 HUB」與「USB2.0 32GB USBDISK」
  + USBDISK:
    + 容量: 32[GB]
    + 格式: exFAT
    + 磁碟機名稱: 與序號相同，例如2025-S005
    + 放置相關軟體至資料夾中
      + R2MS_Lite_安裝軟體:
        + NCUF
        + R2MS_Lite_Driver
      + 其他安裝軟體
        + RustDesk



### SwitchArray 2025-S003
+ SwitchArray完整序號為: 2025-S003
+ 使用「MO系列金屬系統機殼 MO99-16-23B」，並新增支架與面板開孔。
+ 貼上標籤貼紙「2025-S003」。
+ 準備外箱，並貼上貼紙。
+ 準備配件包:
  + USB2.0數據線(透明藍)_1.5m__A公對A公 x1
  + PSU連接線(橘、灰)_槍型接頭轉防水接頭 x1
  + DMM連接線(紅、黑)_槍型接頭公對公 x1
+ SwitchArray內部使用的「ESP32」一律改用「CP2102」版本
  + 使用CP210x修改序號工具將序號修改為: 「2025-S003」
    + 插上裝置後(避免弄錯一次只插上一個CP210x裝置)，運行「CP210xSetIDs.exe」，找到中間有一個格子有標註「Serial Number」，填入序號「2025-S003」，在把前面的勾選框勾起來，最後按下最下方的「Program Device」，完成後關閉修改工具，並反覆插拔裝置與檢查序號是否正確。
  + 修改成功後，可在裝置管理員>連接埠(COM和LPT)>Silicon Labs CP210x USB to UART Bridge (COM3)>詳細資料> 裝置例項路徑，會看到「USB\VID_10C4&PID_EA60\2025-S003」。其中「2025-S003」就是SwitchArray完整序號。
+ SwitchArray內部裝有「USB3.0 HUB」與「USB2.0 32GB USBDISK」
  + USBDISK:
    + 容量: 32[GB]
    + 格式: exFAT
    + 磁碟機名稱: 與序號相同，例如2025-S003
    + 放置相關軟體至資料夾中
      + R2MS_Lite_安裝軟體:
        + NCUF
        + R2MS_Lite_Driver
      + 其他安裝軟體
        + RustDesk
+ 空接放電測試：
  + 主要關心電壓為相似整齊波型，電流為0[A]。
  + 2025-06-06通過測試
+ 10歐姆模擬地層放電測試:
  + 主要關心電壓為相似整齊波型，電流為0.1[A]。
  + 2025-06-06通過測試

### SwitchArray 2025-S007
+ 組裝日期: 2026-02-23
+ SwitchArray完整序號為: 2026-S007
+ 使用「MO系列金屬系統機殼 MO99-16-23B」，並新增支架與面板開孔。。
+ 貼上標籤貼紙:
  + 原廠的最大片覆蓋外殼，選一側為底座，撕掉膠膜，貼上腳座，腳座位置緊靠四個角落。使腳座放在桌上，右側是彎鉤側，在中間底部貼上標籤貼紙「SwitchArray\n2026-S007」。
  + 另一側的最大片覆蓋外殼取出不使用(另外收好)，替換成開孔的覆蓋外殼。內側貼上標籤貼紙「SwitchArray\n2026-S007」於中間下方偏右，貼上標籤貼紙「朝下」於中間下方偏左，貼上標籤貼紙「排針在上方」於中間上方。外側貼上標籤貼紙「SwitchArray\n2026-S007」於中間下方。
  + 原廠的前面板取出不使用(另外收好)，替換成開孔的前面板。內側是指平放時看過去右上角是有四個小孔一大孔的。內側貼上標籤貼紙「面板內側」於中間最下方，再貼上標籤貼紙「SwitchArray\n2026-S007」於中間次下方。
  + 原廠的後面板，貼上標籤貼紙「後面板內側」於中間最下方，再貼上標籤貼紙「SwitchArray\n2026-S007」於中間次下方。
  + 取出支架，貼上標籤貼紙「排針在上方」於中間上方，再貼上標籤貼紙「SwitchArray\n2026-S007」於中間下方。
  + 準備大創透明外箱，並貼上護貝的名稱紙。
+ 若後續零件未就緒，就先組裝起來。記得要把支架鎖到開孔覆蓋外殼中，平頭M4x10螺絲。
+ 準備配件收納包(SC-TI06長方型加大防潑水硬殼收納包):
  + 配件包正反面貼上標籤貼紙「S007」於中間，正反兩面各一張。
  + USB2.0數據線(透明藍)_1.5m_A公對A公，一條。中間貼上標籤貼紙「USB2.0\nAtoA 1.5M」，有加外框字體10pt。兩個USB頭平坦側貼上標籤貼紙「S007」，有加外框字體6pt。用六號夾鏈袋裝好，放入收納包中。
  + PSU連接線(橘、灰)_槍型接頭轉防水接頭，一條。製作方式:
    + 準備槍型接頭(或稱德式香蕉插頭)紅色、黑色，各一個。
    + 從8芯纜線中取出橘色、灰色電線各一條，每條裁切為62[cm]長。
    + 準備KWAY綠色防水接頭一個。平坦側貼上標籤貼紙「S007」，有加外框字體10pt。
    + 先將一端剝線約1[cm]，焊接到槍型接頭上，可用夾子輔助。利用另一個接頭撬開尾部。安裝回去的時候也要利用另一個接頭。紅色接頭配橘色線，黑色接頭配灰色線。
    + 兩條接頭帶線完成後，接到電供上(不要通電)，使用蘋果膠帶把兩條線束緊固定。保留約3[cm]用於安裝防水接頭。
    + 焊接電線到防水接頭上，記得按照順序先把零件套到線上，橘色線焊接到PIN#1，灰色線焊接到PIN#2。最後組裝起來，並使用電錶查驗通路與否。
    + 用五號夾鏈袋裝好，放入收納包中。
  + DMM連接線(紅、黑)_槍型接頭轉電錶接頭，一條。製作方式:
    + 準備槍型接頭(或稱德式香蕉插頭)紅色、黑色，各一個。
    + 從雙頭香蕉接頭測試線(FC-TL2098)中取出黑、紅色各一條，每條120[cm]從中間裁切一半長度，裁完變成約60[cm]長。至此變成黑紅各兩條。
    + 取出裁切好的紅、黑各一條，尾端剝線1.5[cm]，焊接到槍型接頭上，可用夾子輔助。利用另一個接頭撬開尾部。安裝回去的時候也要利用另一個接頭。紅色接頭配紅色線，黑色接頭配黑色線。
    + 兩條接頭帶線完成後，接到電錶上(不要通電)，使用蘋果膠帶把兩條線束緊固定，並貼上標籤貼紙「S007」。兩邊末端保留約3[cm]。
    + 用六號夾鏈袋裝好，放入收納包中。
  + BNC雙公射頻線_1M_RG316(BNC-316U-100)。中間貼上標籤貼紙「S007」，用六號夾鏈袋裝好，放入收納包中。
+ 組裝前面板上的接頭:
  + 安裝一個紅色博士端子頭(優先使用型號9404R 311)
    + 準備配合使用的紅色電源線(從8芯纜線中取出紅色)，長度33[cm]，單端壓接Y型端子(型號Y1.25-3S)並用絕緣膠帶纏繞。最後鎖到博士端子座上。
  + 安裝一個黑色博士端子頭(優先使用型號9404B 311)
    + 準備配合使用的黑色電源線(從8芯纜線中取出黑色)，長度33[cm]，單端壓接Y型端子(型號Y1.25-3S)並用絕緣膠帶纏繞。最後鎖到博士端子座上。
  + 安裝一個雙頭BNC座有絕緣(型號BNC-SS-AI)
  + 安裝一個綠色KWAY防水接頭座
    + 準備配合使用的紅、黑色電源線(從8芯纜線中取出紅、黑色)各一條，每條長度33[cm]。
    + 接頭座編號1處塗上紅色，編號2處塗上黑色。
    + 焊接到接頭座上，最後用熱縮套管加固。 
  + 安裝一個黃色USBA防水接頭座
    + 準備MicroUSB線(優先使用型號IS-C62，長度20[cm])     
+ 組裝上方面板REF接頭:
  + 準備綠色博士端子頭(優先使用型號9404G)
    + 準備配合使用的綠色電源線(從8芯纜線中取出綠色)，長度33[cm]，單端壓接Y型端子(型號Y1.25-3S)並用絕緣膠帶纏繞。最後鎖到博士端子座上。
+ 組裝Control_Board_v1.2
  + 取出一片Control_Board_v1.2，出廠序號為2025100002。移除底部固定螺絲，螺絲另外收好。在H101附近貼上標籤貼紙「SwitchArray\n2026-S007」。
  + 先裝上BNC公轉杜邦母_30cm(型號BNC-MDP-03)，紅色接V_ready，黑色接GND。
  + 再裝上ESP32-WROOM-32E(ESP32-Devkit v3.0)
    + SwitchArray內部使用的「ESP32」一律改用「CP2102」版本
      + 使用CP210x修改序號工具將序號修改為: 「2026-S007」
        + 插上裝置後(避免弄錯一次只插上一個CP210x裝置)，運行「CP210xSetIDs.exe」，找到中間有一個格子有標註「Serial Number」，填入序號「2026-S007」，在把前面的勾選框勾起來，最後按下最下方的「Program Device」，完成後關閉修改工具，並反覆插拔裝置與檢查序號是否正確。
      + 修改成功後，可在裝置管理員>連接埠(COM和LPT)>Silicon Labs CP210x USB to UART Bridge (COM3)>詳細資料> 裝置例項路徑，會看到「USB\VID_10C4&PID_EA60\2026-S007」。其中「2026-S007」就是SwitchArray完整序號。
    + 燒錄韌體「SwitchArray_ver2_9.ino」。
    + 以上工作都完成後，於ESP32上IC的金鐘罩貼上標籤貼紙，上方貼的標籤貼紙「CP2102」有加外框字體6pt。下方貼的是標籤貼紙「S007」有加外框字體10pt。用剪刀稍微修剪再貼上去。
  + 鎖上配合使用的電源線:
    + 牛角座編號1: 準備配合使用的黑色電源線(從8芯纜線中取出黑色)，長度33[cm]，單端壓接Y型端子(型號Y1.25-3S)並用絕緣膠帶纏繞。
    + 牛角座編號2: 準備配合使用的灰色電源線(從8芯纜線中取出灰色)，長度33[cm]，單端壓接Y型端子(型號Y1.25-3S)並用絕緣膠帶纏繞。
    + 牛角座編號3: 準備配合使用的綠色電源線(從8芯纜線中取出綠色)，長度33[cm]，單端壓接Y型端子(型號Y1.25-3S)並用絕緣膠帶纏繞。
    + 牛角座編號4: 準備配合使用的紅色電源線(從8芯纜線中取出紅色)，長度33[cm]，單端壓接Y型端子(型號Y1.25-3S)並用絕緣膠帶纏繞。
    + 牛角座編號5: 準備配合使用的橙色電源線(從8芯纜線中取出橙色)，長度33[cm]，單端壓接Y型端子(型號Y1.25-3S)並用絕緣膠帶纏繞。
+ 組裝RelayBoard_v1.2與Control_Board_v1.2
  + 取出8片RelayBoard_v1.2，並記錄其序號，及其被設定的板子編號:
    + 「RelayBoard\n2026-S007-1」: 序號「2025120071」
    + 「RelayBoard\n2026-S007-2」: 序號「2025120072」
    + 「RelayBoard\n2026-S007-3」: 序號「2025120073」
    + 「RelayBoard\n2026-S007-4」: 序號「2025120074」
    + 「RelayBoard\n2026-S007-5」: 序號「2025120075」
    + 「RelayBoard\n2026-S007-6」: 序號「2025120076」
    + 「RelayBoard\n2026-S007-7」: 序號「2025120077」
    + 「RelayBoard\n2026-S007-8」: 序號「2025120078」
  + 拆封並移除底部固定螺絲，螺絲另外收好。
  + Control_Board_v1.2為第0片，從排針側依序疊好8片RelayBoard_v1.2到一起。
  + 用螺絲鎖到支架上，Control_Board_v1.2朝向支架有挖空處。
+ 組裝整個箱體:
  + 注意上方面板的REF要固定好，博士端子座要鎖緊並且使Y型端子後方的綠線朝前面板方向，避免組裝時壓到線。
  + 對齊RelayBoard到上方面板，之後鎖上螺絲。注意，支架後方先不要鎖緊，等上方面板鎖好之後再補鎖緊支架上的螺絲。
  + 接上MicroUSB線到Control_Board_v1.2上。
  + 安裝前面板:
    + 先把BNC接上
    + 再把USB接上
    + 鎖上前面板固定板
  + 接快接頭(顏色對接)
  + 裝殼，鎖好全部。
  + 完成!
+ 準備相關軟體至NAS資料夾中
  + R2MS_Lite_安裝軟體:
      + NCUF???
      + R2MS_Lite_Driver
  + 其他安裝軟體
    + RustDesk
+ 空接放電測試：
  + 要確認智能控制軟體是否有支援該序號。
  + 主要關心電壓為相似整齊波型，電流為0[A]。
  + 2025-06-06測試失敗???
+ 10歐姆模擬地層放電測試:
  +  要確認智能控制軟體是否有支援該序號。
  + 主要關心電壓為相似整齊波型，電流為0.1[A]。
  + 2025-06-06測試失敗??
